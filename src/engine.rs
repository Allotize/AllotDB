...
impl<'a> std::ops::Drop for KvTxn<'a> {
    fn drop(&mut self) {
        // None represents the  main store
...
/// # fn try_main() -> Result<()> {
/// let mut store = KvStore::open(Path::new("my-store"))?;
    ...
    async fn compact(&mut self) -> Result<()> {
        ...
        for stale_gen in stale_gens {
            ...
            if self.sink.remove_file(&stale_gen).await.is_err() {
                info!("Could not remove stale gen", format!("{:?}", &stale_gen));
            };
        }
...
async fn sorted_gen_list(folder_path: &Path) -> Result<Vec<u64>> {
    ...
    // let log_path_0 = log_path(&path, 0);
